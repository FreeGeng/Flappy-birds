    ...
    : QWidget(parent)
{
    this->data=0;
    this->len=1;
    memset(num,0,sizeof(num)); // set value 0 into num array
    QString str;
    for(int i=0;i<10;i++)
    {
        str.clear();
        str=tr(":/Images/scoreNumber/%1.png").arg(i);
        //tr() is QObject function, it's a tanslator, it will replace %1 with i
        //tr("%1") is similar to print("%d")
        lcdList[i]=str;
    }
    this->setShowHead(true);
    this->setValue(0);
    this->setGeometry(100,100,28*MaxSize,36); //設定相對於父類別的位置 (x,y,width,height)
}
